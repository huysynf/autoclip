...
export interface  tasksProgressUpdateMessage extends WebSocketMessage {
  ...
  step_name: string;
  message?: string;
  snapshot?: boolean; // Mark YesNo as snapshot Message
}
...
// Global WebSocket connectionManage
...
const HEARTBEAT_INTERVAL = 25000; // Send a heartbeat every 25 seconds
const HEARTBEAT_TIMEOUT = 5000; // Reconnect if no pong is received within 5 seconds

// Anti-shake mechanism
let syncDebounceTimeout: number | null = null;
const SYNC_DEBOUNCE_DELAY = 300; // 300ms anti-shake
...
export const useWebSocket = (options: UseWebSocketOptions) => {
    ...
    heartbeatInterval = window.setInterval(() => {
      if (globalWs?.readyState === WebSocket.OPEN) {
        ...
        heartbeatTimeout = window.setTimeout(() => {
          console.log('Heartbeat Timeout, ready to reconnect');
          if (globalWs) {
            globalWs.close();
          }
        }, HEARTBEAT_TIMEOUT);
  ...
  const ensureConnected = useCallback(() => {
    ...
    // If there is already a connection but the UserID is different, close the old connection first.
    if (globalWs && globalUserId !== userId) {
      console.log(`UserID change: ${globalUserId} -> ${userId}, Close old connection`);
      globalWs.close();
      globalWs = null;
    }
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        // Project List has been refreshed
        startHeartbeat();
        
        // Auto resubscribes to previous projects after reconnection
        if (globalDesiredSubscriptions.size > 0) {
          console.log('Re-subscribe to projects after reconnecting:', Array.from(globalDesiredSubscriptions));
          sendMessage({
      ...
      ws.onmessage = (event) => {
        try {
          ...
          // Processingpong response
          if ((data.type as any) === 'pong') {
            console.log('Heartbeat pong response received');
            if (heartbeatTimeout) {
              clearTimeout(heartbeatTimeout);
              heartbeatTimeout = null;
            }
          ...
          console.error('Parsing WebSocket messageFailed:', error);
        }
      };
      ...
      ws.onclose = (event) => {
        ...
        // EnableAuto reconnection, but limit the number of reconnections
        if (event.code !== 1000 && reconnectAttempts < maxReconnectAttempts) {
          reconnectAttempts++;
          const delay = Math.min(2000 * Math.pow(2, reconnectAttempts), 15000);
          console.log(`Will attempt to reconnect after ${delay}ms (${reconnectAttempts}/${maxReconnectAttempts})`);
          
          reconnectTimeoutRef = setTimeout(() => {
            ensureConnected();
          }, delay);
        } else if (reconnectAttempts >= maxReconnectAttempts) {
          console.log('When the Max number of reconnections is reached, Stop reconnection.');
        }
      };
  ...
  const disconnect = useCallback(() => {
    ...
    if (globalWs) {
      globalWs.close(1000, 'User actively Disconnected');
      globalWs = null;
    }
  ...
  const sendMessage = useCallback((message: any) => {
    ...
    console.warn('WebSocket is not connected, None method Message sent');
    return false;
  }, []);
  ...
  // Collection difference alignment subscription - core function (with anti-shake)
  const syncSubscriptions = useCallback((projectIds: string[]) => {
    ...
    // RefreshB station Page to capture the request
    if (syncDebounceTimeout) {
      clearTimeout(syncDebounceTimeout);
    }
    
    syncDebounceTimeout = window.setTimeout(() => {
      // Send synchronous subscription request
      if (globalWs?.readyState === WebSocket.OPEN) {
        console.log('Synchronously subscribe to projects:', Array.from(desired));
        sendMessage({
          type: 'sync_subscriptions',
  ...
  // Bulk subscription/unsubscription support
  ...
  // Synchronous subscription set alignment interface
  ...
  // Auto connection
  useEffect(() => {
    // Delay connection to avoid re-rendering during ComponentInitializing
    ...
    return () => {
      clearTimeout(timer);
      // Not Disconnected here, keep globally connected
    };
