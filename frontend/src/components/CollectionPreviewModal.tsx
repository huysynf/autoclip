...
const CollectionPreviewModal: React.FC<CollectionPreviewModalProps> = ({
  ...
  // 从storeMedium获取最新的collectionStatus
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    // 显示加载Status
    const hideLoading = message.loading('正在Update切片顺序...', 0)
    setIsUpdating(true)
  ...
  const handleRemoveClip = async (clipId: string) => {
    ...
    const hideLoading = message.loading('正在Remove切片...', 0)
    setIsUpdating(true)
  ...
  const handleAddClips = async (selectedClipIds: string[]) => {
    ...
    const hideLoading = message.loading('正在Add切片...', 0)
    setIsUpdating(true)
  ...
  return (
    ...
    >
      ...
      <UploadModal
        ...
        onSuccess={() => {
          // UploadSuccess后可以Refresh数据或显示提示
