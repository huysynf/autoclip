...
const BilibiliDownload: React.FC<BilibiliDownloadProps> = ({ onDownloadSuccess }) => {
  ...
  const parseVideoInfo = async () => {
    ...
    if (!videoType) {
      setError('Please enter a valid Bilibili or YouTube video URL')
      return
    }
  ...
  const handleDownload = async () => {
    ...
    const videoType = getVideoType(url.trim())
    if (!videoType) {
      message.error('Please enter a valid Bilibili or YouTube video URL')
      return
    }
    ...
    <div style={{
      ...
      margin: '0 auto'
        ...
        <Space direction="vertical" style={{ width: '100%' }} size={16}>
          <div>
            <Input.TextArea
              placeholder="Paste a Bilibili or YouTube video URL here. Supported: • Bilibili: https://www.bilibili.com/video/BV1xx411c7mu • YouTube: https://www.youtube.com/watch?v=xxxxx"
              value={url}
              onChange={(e) => {
          ...
          {videoInfo && (
            <>
              ...
              <div>
                ...
                <Select
                  placeholder="Select a browser to retrieve cookies (optional)"
                ...
                <Text style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '12px', marginTop: '8px', display: 'block' }}>
                  Selecting a browser allows access to AI subtitles using your login session. Without it, only public subtitles can be downloaded.
                </Text>
              </div>
