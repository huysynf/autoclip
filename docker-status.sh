...
# Version: 1.0
# Purpose: Check AutoClip Docker service status
...
check_services() {
    ...
    if curl -fsS "http://localhost:8000/api/v1/health/" >/dev/null 2>&1; then
        ...
        log_error "Backend API is unhealthy"
    fi
...
check_resources() {
    ...
    docker stats --no-stream --format "table {{.Container}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.NetIO}}\t{{.BlockIO}}" $(docker ps --filter "name=autoclip" --format "{{.Names}}" 2>/dev/null || true) 2>/dev/null || log_warning "Unable to retrieve resource usage"
}
...
main() {
    ...
    if [[ $overall_status -eq 0 ]]; then
        ...
        echo -e "  3. Full restart:       ./docker-start.sh"
    fi
}
